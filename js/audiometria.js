$(document).ready(function(){
  jQuery.ajax({
    url: `http://${_DOMAIN_SERVICES}/autodiagnostico-rest-services/questionservice/getquestion/${_AUDIOMETRIA}`
  })
  .done(function(data){
    if(data.response == 200){
      console.log(data);
      let form = $('#form-audiometria');
  ...
  $('#form-audiometria').on("submit", function(e){
    e.preventDefault();
    swal({
...
function getFormstate(){
  ...
  return jQuery.ajax({
    url: `http://${_DOMAIN_SERVICES}/autodiagnostico-rest-services/formstservice/getformstfield/${idUser}/${_AUDIOMETRIA}`,
...
function validateAnswers(){
  if ($('#21').val() == '46' && $('#22').val() == '48' && $('#26').val() == '56'){
    return 'NO_APROBADO';
  }
  if ($('#20').val() == '43' && $('#21').val() == '47' && $('#22').val() == '49' && $('#23').val() == '51' && $('#25').val() == '55'){
    return 'APROBADO';
  }
...
function downloadPDF(){
  window.open(`http://${_DOMAIN_SERVICES}/autodiagnostico-rest-services/certificateservice/getPdf/${sessionStorage.getItem('numeroDocumento')}/${_AUDIOMETRIA}`, '_blank');
}